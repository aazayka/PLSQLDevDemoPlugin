...
begin
  //20 = ShowWord
  IDE_CreatePopupItem(PlugInID, 20, menuName, 'SQLWINDOW');
  IDE_CreatePopupItem(PlugInID, 20, menuName, 'COMMANDWINDOW');
  IDE_CreatePopupItem(PlugInID, 20, menuName, 'TESTWINDOW');
  IDE_CreatePopupItem(PlugInID, 20, menuName, 'COMMANDWINDOW');
...
function getSynonymByTable(Owner, TableName: string): string;
var
  Query: string;
  index: Integer;
begin
...
function getTableBySynonym(Owner, TableName: string): string;
var
  Query: string;
  index: Integer;
begin
  Query := 'Select  from all_synonyms '
      + ' WHERE synonym_owner = UPPER(NVL(''' + Owner + ''', synonym_owner))'
      + ' AND synonym_name = UPPER(''' + TableName + ''')';
  SQL_Execute(PChar(Query));
  index := SQL_FieldIndex('SYNONYM_NAME');
  // Get first row. If several rows are expected -- rewrite query to get the most valuable result first (or aggregate results)
  if not SQL_Eof then
...
function GetSingleWord(Str: string): string ;
begin
  if Str <> '' then
    begin
      Result := 'Look for synonym for table = ' + Str;
      if Result = '' then
        begin
          Result := 'Look for table for synonym = ' + Str;
        end;
    end
  else
    begin
       Result := 'Empty parameter for SingleWord';
    end;

end;

function GetTwoWords(Str1: string; Str2: string): string ;
begin
  if (Str1 <> '') and (Str2 <> '') then
    begin
      Result := 'Look for synonym for table owner ' + Str1 + ' table name = ' + Str2;
      if Result = '' then
        begin
          Result := 'Look for synonym for synonym owner ' + Str1 + ' synonym name = ' + Str2;
        end;
    end
  else if (Str1 <> '') or (Str2 <> '') then
     Result := GetSingleWord(Str1 + Str2)
  else
    Result := 'Empty parameter for 2 words';

end;

procedure ProcessSelectedText(const Msg: string);
  var MessageToShow: string;
begin
    ShowMessage('Is called for ' + Msg);
    
     words := TStringList.Create;
     Split('.', Msg, words);
     ShowMessage(words.CommaText);

     if words.Count = 1 then
     //It's a table name or Synonym name
      begin
        MessageToShow := GetSingleWord(words.ValueFromIndex[0]);
      end
     else if words.Count = 2 then
     //Schema.Table or Schema.Synonym
      begin
        MessageToShow := GetTwoWords(words.ValueFromIndex[0], words.ValueFromIndex[1]);
      end;
...
begin
  case Index of
   ...
   19 : ProcessSelectedText(IDE_GetSelectedText);
   20 : ProcessSelectedText(IDE_GetCursorWord);
